...
from maquis.utils.corrutils import merge_transpose as assy_c
...
class MaquisMeasurement:
    ...
    def __init__(self, inputfile):
        self.loc_nup = assy_vec(loadEigenstateMeasurements([inputfile], what='Nup')[0][0])
        self.loc_ndown = assy_vec(loadEigenstateMeasurements([inputfile], what='Ndown')[0][0])
        self.loc_docc = assy_vec(loadEigenstateMeasurements([inputfile], what='Nupdown')[0][0])

        self.norb = len(self.loc_nup)
        empty_diag = np.zeros(self.norb)

        self.corr_cdag_up_c_up = assy_hc(empty_diag, loadEigenstateMeasurements([inputfile], what='dm_up')[0][0])
        self.corr_cdag_down_c_down = assy_hc(empty_diag, loadEigenstateMeasurements([inputfile], what='dm_down')[0][0])

        self.corr_nupnup =     assy_hc(empty_diag, loadEigenstateMeasurements([inputfile], what='nupnup')[0][0])
        self.corr_nupndown =   assy_hc(empty_diag, loadEigenstateMeasurements([inputfile], what='nupndown')[0][0])
        self.corr_ndownnup =   assy_hc(empty_diag, loadEigenstateMeasurements([inputfile], what='ndownnup')[0][0])
        self.corr_ndownndown = assy_hc(empty_diag, loadEigenstateMeasurements([inputfile], what='ndownndown')[0][0])

        self.corr_docc =       assy_hc(empty_diag, loadEigenstateMeasurements([inputfile], what='doccdocc')[0][0])
        self.corr_trans_up =   assy_hc(empty_diag, loadEigenstateMeasurements([inputfile], what='transfer_up_while_down')[0][0])
        self.corr_trans_down = assy_hc(empty_diag, loadEigenstateMeasurements([inputfile], what='transfer_down_while_up')[0][0])

        u1 = loadEigenstateMeasurements([inputfile], what='transfer_up_while_down_at_2')[0][0]
        u2 = loadEigenstateMeasurements([inputfile], what='transfer_up_while_down_at_1')[0][0]
        d1 = loadEigenstateMeasurements([inputfile], what='transfer_down_while_up_at_2')[0][0]
        d2 = loadEigenstateMeasurements([inputfile], what='transfer_down_while_up_at_1')[0][0]
        ...
        self.corr_trans_pair = assy_hc(empty_diag, loadEigenstateMeasurements([inputfile], what='transfer_pair')[0][0])

        self.corr_spinflip = assy_hc(empty_diag, loadEigenstateMeasurements([inputfile], what='spinflip')[0][0])

        u1 = loadEigenstateMeasurements([inputfile], what='nupdocc')[0][0]
        u2 = loadEigenstateMeasurements([inputfile], what='doccnup')[0][0]
        self.corr_nupdocc = assy_c(empty_diag, u1, u2)
        self.corr_doccnup = assy_c(empty_diag, u2, u1)

        u1 = loadEigenstateMeasurements([inputfile], what='ndowndocc')[0][0]
        u2 = loadEigenstateMeasurements([inputfile], what='doccndown')[0][0]
