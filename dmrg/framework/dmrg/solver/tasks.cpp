...
#include "dmrg/solver/numeric/axpy_template.h"
...
namespace detail{
    ...
    template <class VT>
    typename Cohort<VT>::index_type Cohort<VT>::get_lb() const { return lb; }
    template <class VT>
    typename Cohort<VT>::index_type Cohort<VT>::get_rb() const { return rb; }
    ...
    template <class T>
    typename MPSBlock<T>::const_iterator MPSBlock<T>::begin() const { return data.begin(); }
    template <class T>
    typename MPSBlock<T>::const_iterator MPSBlock<T>::end() const { return data.end(); }
    template <class T>
    typename MPSBlock<T>::iterator MPSBlock<T>::begin() { return data.begin(); }
    template <class T>
    typename MPSBlock<T>::iterator MPSBlock<T>::end() { return data.end(); }
...
///////////////////////////////////////////////////////////////////////////////////////////////
    ...
    {
        for (std::vector<WorkSet<value_type>> const& dev_pipeline : pipeline)
            for (WorkSet<value_type> const & ws : dev_pipeline)
...
template <class T> size_t ScheduleNew<T>::size() const {
    return mpsblocks.size();
}
template <class T> typename ScheduleNew<T>::base::iterator ScheduleNew<T>::begin() {
    return mpsblocks.begin();
}
template <class T> typename ScheduleNew<T>::base::iterator ScheduleNew<T>::end() {
    return mpsblocks.end();
}
template <class T> typename ScheduleNew<T>::base::const_iterator ScheduleNew<T>::cbegin() const {
    return mpsblocks.cbegin();
}
template <class T> typename ScheduleNew<T>::base::const_iterator ScheduleNew<T>::cend() const {
    return mpsblocks.cend();
}
